...
import com.sarius.germanpixeldungeon.ui.CheckBox;
...
public class WndSettings extends Window {
	...
	public WndSettings( boolean inGame ) {
		...
		if (inGame) {
			...
			CheckBox btnScaleUp = new CheckBox( TXT_SCALE_UP ) {
				...
				protected void onClick() {
					super.onClick();
					GermanPixelDungeon.scaleUp( checked() );
				}
			};
			btnScaleUp.setRect( 0, 0, WIDTH, BTN_HEIGHT );
			btnScaleUp.checked( GermanPixelDungeon.scaleUp() );
			add( btnScaleUp );
			...
			btnImmersive = new CheckBox( TXT_IMMERSIVE ) {
				...
				protected void onClick() {
					super.onClick();
					GermanPixelDungeon.immerse( checked() );
				}
			};
			btnImmersive.setRect( 0, btnScaleUp.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnImmersive.checked( GermanPixelDungeon.immersed() );
		...
		CheckBox btnSound = new CheckBox( TXT_SOUND ) {
			...
			protected void onClick() {
				super.onClick();
				GermanPixelDungeon.soundFx( checked() );
				Sample.INSTANCE.play( Assets.SND_CLICK );
			}
		};
		btnSound.setRect( 0, btnMusic.bottom() + GAP, WIDTH, BTN_HEIGHT );
		btnSound.checked( GermanPixelDungeon.soundFx() );
		add( btnSound );
		...
		if (!inGame) {
			...
			RedButton btnOrientation = new RedButton( orientationText() ) {
				@Override
				protected void onClick() {
					GermanPixelDungeon.landscape( !GermanPixelDungeon.landscape() );
				}
			};
			...
			CheckBox btnBrightness = new CheckBox( TXT_BRIGHTNESS ) {
				...
				protected void onClick() {
					super.onClick();
					GermanPixelDungeon.brightness( checked() );
				}
			};
			btnBrightness.setRect( 0, btnSound.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnBrightness.checked( GermanPixelDungeon.brightness() );
			add( btnBrightness );
	...
	private void zoom( float value ) {
		...
		GermanPixelDungeon.zoom( (int)(value - PixelScene.defaultZoom) );
	...
	private String orientationText() {
		return GermanPixelDungeon.landscape() ? TXT_SWITCH_PORT : TXT_SWITCH_LAND;
	}
}
